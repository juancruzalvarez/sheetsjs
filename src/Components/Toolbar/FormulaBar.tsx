...
const FormulaBar: React.FC = () => {
  ...
  const cellData = useSpreadsheetStore(
    useCallback((state) => 
      currentCell ? state.cells.get(`${currentCell.row}-${currentCell.col}`) : null,
      [currentCell]
    )
  );
      ...
      <div className="flex-1 flex items-center gap-1 border border-gray-300 rounded overflow-hidden focus-within:border-blue-400">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (currentCell) {
              setEditingState(currentCell.row, currentCell.col, e.target.value);
            }
          }}
          onFocus={() => {
            if (currentCell) {
              setEditingState(currentCell.row, currentCell.col, formulaBarValue);
            }
          }}
          onBlur={() => {
